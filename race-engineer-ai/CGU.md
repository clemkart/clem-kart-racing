...
Le Service est édité par Clément DANIEL, créateur indépendant, et propose à ses utilisateurs des conseils techniques, des analyses de session, des recommandations de réglages châssis et un accompagnement pédagogique au pilotage karting, le tout assisté par intelligence artificielle.

L'utilisation du Service implique l'acceptation pleine et entière des présentes CGU par l'Utilisateur. Si l'Utilisateur n'accepte pas tout ou partie de ces CGU, il doit renoncer à utiliser le Service.
...
- **Compte** : espace personnel sécurisé créé par l'Utilisateur via inscription par email (magic link), permettant la persistance de ses données dans le cloud.
- **Contenu** : tout texte, conseil, recommandation, analyse, diagnostic, code ou élément multimédia produit par le Service ou par les contributeurs.
- **Données pilote** : informations renseignées par l'Utilisateur concernant son profil (taille, poids, catégorie, châssis, moteur, style, niveau) et ses sessions (conditions, ressenti, réglages, chronos).
- **IA** : intelligence artificielle (modèle de langage Claude par Anthropic, utilisé via API) qui génère les analyses et recommandations.
...
- Pour les Utilisateurs inscrits : la création d'un Compte, via validation de la case à cocher « J'accepte les CGU » lors de la première connexion.

L'acceptation est sans réserve. Toute utilisation conditionnée ou partielle est interdite.

L'Utilisateur reconnaît avoir lu, compris et accepté l'intégralité des présentes CGU avant utilisation.
...
- Analyse automatisée du comportement du kart (sous-virage, survirage, instabilité, etc.)
- Recommandations de réglages châssis assistées par IA
- Chat conversationnel avec un assistant IA spécialisé karting
- Diagnostic guidé en plusieurs étapes
...
- De la disponibilité des services tiers (Anthropic, Supabase, Netlify)
- D'éventuelles interruptions pour maintenance, mises à jour ou incidents techniques
...
- Ajouter, retirer ou modifier des options gratuites ou payantes
- Modifier l'apparence et l'ergonomie
- Suspendre temporairement ou définitivement tout ou partie du Service
...
- Ne pas créer plusieurs Comptes avec la même identité
- Ne pas partager son accès avec un tiers
...
Ils sont générés par une intelligence artificielle (modèle Claude d'Anthropic) à partir des informations renseignées par l'Utilisateur et d'une base de connaissance pédagogique. Ils ne constituent en aucun cas :
- Un avis d'expert mécanicien professionnel
- Une garantie de performance, de chronos ou de résultat sportif
...
- De la vérification mécanique avant et après chaque session
- De l'application ou non des recommandations du Service
- Des conséquences éventuelles d'un réglage modifié (casse matériel, accident, perte de performance)
- De l'usage qu'il fait du Service sur la piste

**L'Éditeur décline expressément toute responsabilité en cas de :**
- Casse moteur, châssis, pneus, ou tout autre élément du kart
- Accident, blessure, chute, collision sur piste ou en transport
...
- L'absence d'erreurs dans la base de connaissance
- L'adéquation des recommandations à la situation particulière de l'Utilisateur
...
- Assurer la sécurité du Service (anti-fraude, rate limiting)
...
- **Supabase Inc.** (États-Unis, hébergement Europe disponible) : pour le stockage des Comptes et sessions cloud
- **Netlify, Inc.** (États-Unis) : pour l'hébergement du Service et l'exécution des Functions

Les transferts hors UE sont encadrés par les Clauses Contractuelles Types de la Commission européenne ou des certifications équivalentes (Data Privacy Framework).
...
- **Droit à l'effacement** : suppression de son Compte et de ses données
...
- **Droit de réclamation** : auprès de la CNIL (Commission Nationale de l'Informatique et des Libertés, www.cnil.fr)
...
- Row Level Security (RLS) sur la base de données (chaque Utilisateur ne voit que ses propres données)
- Connexions chiffrées HTTPS / TLS
- Clés API stockées en variables d'environnement (jamais dans le code)

En cas de violation de données susceptible d'engendrer un risque pour les droits et libertés de l'Utilisateur, l'Éditeur s'engage à notifier l'incident à la CNIL et à informer l'Utilisateur dans les délais prévus par le RGPD.
...
- Bloquer les cookies dans son navigateur (mais cela peut altérer le fonctionnement)
- Se déconnecter manuellement pour mettre fin à la session
...
Les Utilisateurs inscrits seront informés des modifications substantielles par email ou par notification dans le Service au moins 15 jours avant leur entrée en vigueur.

L'utilisation du Service après l'entrée en vigueur des nouvelles CGU vaut acceptation. Si l'Utilisateur n'accepte pas les modifications, il doit cesser d'utiliser le Service et peut demander la suppression de son Compte.

La version applicable est celle en vigueur à la date d'utilisation. La date de dernière mise à jour est indiquée en haut du présent document.
...
- La suppression des données associées (sessions, profil cloud) sous 30 jours
- L'extinction de toutes obligations de l'Éditeur envers l'Utilisateur

Les données stockées localement (localStorage) ne sont pas affectées par la résiliation du Compte cloud. L'Utilisateur doit les effacer manuellement s'il le souhaite.
...
Pour les Utilisateurs non consommateurs (professionnels, équipes, structures), compétence exclusive est attribuée aux tribunaux du ressort du domicile de l'Éditeur, sauf disposition d'ordre public contraire.
