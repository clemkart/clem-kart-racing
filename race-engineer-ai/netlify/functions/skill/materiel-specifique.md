...
- ⚠️ **À confirmer** : connaissance générale ou déduction, non validée par source fiable ni expert
- 👤 **Validé Clément** : confirmé par l'expérience directe du fondateur

**RÈGLE IA** : info ⚠️ = signaler que c'est indicatif, inviter à vérifier auprès du préparateur/concessionnaire. Ne JAMAIS présenter une info ⚠️ comme une vérité. Si pas de data fiable pour un combo → privilégier la physique générale et le dire honnêtement.
...
### CRG
- ✅ Un des 3 grands fabricants mondiaux.
- ✅ **Barre de torsion** : installée, donne plus de grip arrière (empêche le châssis de transférer le poids / lever la roue intérieure autant) → le kart redescend et accélère plus tôt.
- ✅ **Barre tubulaire : partie plate horizontale = flexible, tournée verticale = beaucoup plus rigide** (confirme la logique d'orientation de barre).
- ✅ Châssis soft (axe, barre souples) = moins de grip. Plus rigide = plus de grip, avec point de diminishing returns (trop ferme = ne performe plus).
- ✅ **Voie arrière** : règle = aussi large que possible. CRG conçu pour max 55 pouces. Grip max vers 54". Passer aux moyeux courts avant de réduire sous 54.5", par incréments de 1/8". Châssis très réactif aux petits changements.
- ✅ **Réglages front-end CRG (manuel officiel SSC Racing)** : caster/camber baseline = **II en haut / II en bas** aux fusées. Voie avant **45,5" à 46"**. **Pare-chocs avant TOUJOURS serré.** Barres de side pods desserrées (mais boulons serrés). Plus de caster sur le week-end pour combattre l'usure pneu / la perte de grip.
- ✅ **Correction sous-virage CRG** : écarter d'1 spacer de roue sur les 2 fusées. Si l'avant devient trop large → revenir + changer caster/camber de II/II vers II/III ou III/III (re-vérifier le pincement après).
- ⚠️ Réputé légèrement plus "arrière" qu'OTK (à confirmer).

### Birel ART
- ✅ Un des 3 grands fabricants mondiaux, caractéristiques différentes d'OTK.
- ⚠️ Réputé comportement neutre, sensible aux pressions pneus.
- ⚠️ Système de pré-charge AV selon modèles.
- ✅ **CONFIRMÉ par sources web** : Birel ART = **fenêtre de réglage tolérante/forgiving, équilibre prévisible, facile à travailler sur une large plage de grip.** Plateforme stable et rassurante qui récompense les inputs smooth et génère une bonne traction en sortie. Fort sur grip medium.
- ✅ **Grip latéral** (confirme le retour de Clément !) : "l'axe arrière offre un très bon grip latéral, bonne stabilité au freinage et traction optimale tout en laissant le kart très libre."
- ✅ **Structure** : solution tube 32mm = structure plus rigide → meilleures traction et grip (dépend aussi des pneus).
- ✅ **Setup haut grip Birel** : axe avant court (~1000mm) + arrière large (~1400mm).
- 👤 **Retour Clément (confirmé)** : très bon grip latéral + se différencie sous forte chaleur (tient mieux quand il fait chaud, là où d'autres décrochent). → Recommandation : sur combo Birel + forte chaleur, c'est un point fort à exploiter.

### Parolin
- ✅ Marque italienne majeure, très performante (podiums + victoires 2024 dont WSK Final Cup).
- ✅ **Adaptabilité supérieure à toutes conditions de grip** (forme cadre + type de tubes).
- ✅ Nombreux réglages : axe permettant d'ajuster facilement la hauteur de caisse + système de direction avant propriétaire.
- ✅ Châssis Le Mans adapté TAG, Rotax & X30.
- ✅ **Le Mans** : tubes acier CrMo Ø30mm (+ tube avant 32mm). Idéal grip MEDIUM, les longerons 30mm génèrent un comportement torsionnel bénéfique pour les catégories direct drive. **Opportunity** (Ø28mm) = adaptabilité supérieure toutes conditions.
- ✅ **Systèmes de réglage Parolin** : "Easy Caster System" (caster + camber réglables indépendamment, rapidement) · support de colonne de direction réglable (angle Ackermann modifiable selon circuit/style) · cassette de palier d'axe arrière à 4 vis (réglage hauteur, le plus d'options possibles).
- ⚠️ Souvent souple, privilégie la rotation.

### Kart Republic (KR)
- ✅ Marque récente world-class (victoires Europe/USA/Asie), principal rival d'OTK en OK/100cc.
- ✅ **KR2 : mix de tubes 30mm et 32mm**, toutes catégories single-speed (KA100 → Rotax/X30 → OK).
- ✅ "Facile à régler, s'adapte au mieux à tous circuits et pneus."
- ⚠️ Terminologie "tube" AV. Garder la logique souplesse/rigidité.
- 👤 **Retour Clément (indirect, pas testé)** : châssis très performant qui **se conduit avec une glisse contrôlée** (style de pilotage légèrement différent, à l'aise dans le glissement maîtrisé). À approfondir par recherche web.
...
- ✅ **EGT recommandé : 1050-1100°F** (sonde à l'emplacement IAME). Surveiller aussi température eau.
- ✅ **Fort couple bas et mi-régime** → efficace sur **circuits techniques/serrés** (accélération sortie de virages lents > vitesse de pointe).
...
- ✅ **Gearing** : couronne arrière typiquement **70-80 dents**, pignon avant **10-11 dents**. Plus de dents couronne (ratio haut) = accélération (circuits courts/techniques). Moins de dents (ratio bas) = vitesse de pointe (longues lignes droites).
- ✅ Motoristes KZ2 : IAME Screamer III, TM KZ-R2, Vortex VTZ, Modena KK3.
...
### Rok GP (Vortex)
- ✅ **ROK ~36 chevaux** (plus puissant que X30 et Rotax selon une source).
- ⚠️ Famille Vortex Rok : Mini Rok, Rok Junior, Rok Senior, Shifter Rok. Préciser selon variante.

### Mini Swift / Micro Swift
- ⚠️ Petits moteurs catégories jeunes. Puissance réduite, réglages limités.
...
- ⚠️ Châssis "adaptable" (Parolin, KR) : tolérant aux changements, bon pour pilotes débutant le réglage.
- ✅ Philosophie OTK (rigidifier sur bas grip pour forcer le délestage) : valide pour OTK. Pour les autres marques, la même logique tient généralement (CRG le confirme via barre/axe), mais l'ampleur varie.

### Moteur × Circuit
- ✅ **X30 → circuits techniques** (couple bas régime). Carbu ajustable selon T° air.
- ✅ **Rotax Max → circuits rapides** (mid-high range). Carbu fixe = focus châssis.
...
- ⚠️ Châssis "front-biased" (OTK) + pilote "agressif au freinage" = risque de sur-solliciter l'avant → conseiller la finesse.
- ⚠️ Châssis souple (Parolin) + pilote "finesse en glisses" = combo naturel pour la rotation.
- ✅ Gros moteurs (KZ, DD2) → siège reculé pour planter les roues motrices. Petits moteurs → siège plus avancé/centré pour faciliter la rotation (cf. reglages-detailles.md § Position siège).

---

## CE QUI MANQUE (à enrichir par Clément 👤)
1. Retours directs sur OTK, Sodikart, Rotax Max, DD2 (expérience terrain = la source la plus fiable)
2. Spécificités de réglage propres à chaque marque qu'il connaît
3. Comportements observés par condition réelle

Pour tout ce qui n'est ni ✅ ni 👤, rester prudent et privilégier la physique générale (vision-pilote.md + principes-universels.md + mecanique-kart-specifique.md).

---

## Sources web (2026-05)
forums.kartpulse.com · kartclass.com · tkart.it · motorsportmalta.com · ekartingnews.com · kartwiki.com · CRG/OTK setup guides (PDF) · nashvillekartinggroup.com · grokipedia (KZ2) · vroomkart.com · kartsportnews.com
